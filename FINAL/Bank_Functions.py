...
def checkPassword(i, pword):
    # returns true so that the account will be inaccessible
    if pword == "blocked":
...
def transferbetweeen(prompt, recipient, current, customerdata, recipientbank):
    ...
    check = next((True for i in temp if i["name"] == recipient), False)
    if check:
        ...
        if confirmer():
            ...
            print(
                "\nYou have transferred Rp. %s to %s of %s bank"
                % (transfer, recipient, recipientbank)
            )
            # class function is called in order to print a balance statement
        ...
        print("The recipient is not registered in that bank")
...
def transferlocal(recipient, customerdata, customers, cname, current):
    ...
    check = next(
        (True for i in customerdata.exportcustomers() if i["name"] == recipient), False
    )
    if check:
        ...
        print("Recipient is not registered in our database")
...
def Hashed(string, name, banks, salt=None):
    ...
    if salt == None:
        ...
        salt = next(i["hasher"].encode() for i in banks if i["name"] == name)
    try:
    ...
    while True:
        ...
        if success == True:
            ...
            if admin:
                ...
                print("\nThat user is not registered in this bank")
                continue
            else:
                quit = str(
                    input(
